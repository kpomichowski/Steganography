...
class TextLSB:
  ...
  def __encode(self, image: List[Image.Image], text: str):
    ...
    index = 0
    for count, (rgb_bin, ascii_bit) in enumerate(zip_longest((bin_ for channel in rgb_channels for bin_ in channel), ascii_codes), start=0):

      if count % 3 == 0 and count != 0:
        index += 1

      if ascii_bit != None:
        if ascii_bit == rgb_bin[-1]: continue
